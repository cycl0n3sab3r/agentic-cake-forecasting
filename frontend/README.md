...
Not started yet. Placeholder so the directory is tracked.
...
is no server to run and it can go on GitHub Pages.

Suggested first step: add `scripts/export_json.py` writing to `frontend/public/`,
then pick a framework once the data contract is stable.
